...
| Review a finished week: results, bench points, start/sit calls, and the week's moves | `POST /api/v1/recap` | `get_recap` |

All six capabilities are read-only with respect to fantasy rosters and external communication. Notification settings and delivery remain owner-controlled; analysis calls do not send alerts or submit trades, claims, or lineup changes. Search covers one-for-one swaps and two-for-one packages in either direction; manual evaluation supports any package, choosing roster drops where a side ends up over its limit.
