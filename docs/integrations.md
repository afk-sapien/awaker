...
4. Open **Notifications** in the sidebar. The page shows the configured account and the leagues it covers. Choose daily and weekly summaries, the timezone, and alert thresholds. Every change saves as you make it. Alerts follow the filters set on the Auto trades and Waiver wire pages and the leagues turned on in My leagues. Schedules and alerts start disabled.
5. To receive pushes, fill in **Your phone** on the same page. No ntfy account is needed: keep the default `https://ntfy.sh` server, press **Random** for a topic, subscribe to the shown address in the ntfy app, and use **Send test**, which reports whether ntfy accepted it and why not. On a public server the topic name is the only secret, so keep it unguessable. A self-hosted server and an access token for a protected topic are optional. The browser can choose any HTTPS ntfy server. A self-hosted server on plain HTTP, including one on loopback or the local network, works only when `NTFY_URL` in the service's environment names that exact scheme, host and port, so a browser session cannot aim deliveries and their token at an address the operator did not choose. URLs with a fragment are rejected, and failed test sends say whether the server was unreachable or refused the message without repeating its status code. Changes apply immediately, without a restart. Without ntfy, scheduled reports are archived in the app.
...
Opportunity scans run every 1, 3, 6, 12, or 24 hours (default six) when trade or waiver alerts are enabled, and **Check now** runs one on demand under the same rules. Trades and waiver pickups have separate toggles and thresholds. A trade must pass the owner's existing minimum gain and value-balance rules and the trade threshold (default +3 projected points/week). A waiver pickup comes from the same engine as the waiver view and `get_opportunities`, honoring protected players and drop rules, and must clear the pickup threshold (default +3 projected points, judged for this week, next week or per week over the rest of the season). Bench upgrades, off by default, add the best pickup per league that would not start but projects above a bench player the lineup never needs. The first check after alerts are turned on is silent; later weeks keep alerting, and a failed run backs off from two minutes up to an hour. When several free agents would replace the same drop, only the best one counts.

The first complete non-demo scan of each kind establishes a baseline without notifying. A new NFL week keeps that baseline, so pickups found right after the week turns are sent, and each opportunity is still sent once per key. After that an opportunity is new when its league, players, and partner or drop have not been seen in the current scans. Repeat alerts require a material improvement (default +2 points), a six-hour cooldown, and room under the daily cap (default three). Everything new in one scan is sent as **one** notification, such as "Awaker: 2 trades, 1 waiver pickup", listing the top five by projected gain and linking to the relevant view, and it counts once against the daily cap. A scan during quiet hours holds its findings and rescans in the first minute after they end. Findings held by the daily cap or a missing ntfy connection go out with a later scan. If one kind's data is incomplete, that kind is skipped and the other still runs. Changing the account, schedule or timezone resets the baseline and cancels queued alerts and reports. Preference, filter and alert-setting changes keep the baseline and queued alerts, so anything that newly qualifies is sent rather than silently absorbed. **Background activity** shows the last scan, what it found, and when the next is due.
