...
`AWAKER_HOST` defaults to loopback. `PORT` defaults to 4173. Update `AWAKER_PUBLIC_URL` and `AWAKER_API_URL` if you change it. `SLEEPER_USERNAME` fixes the account the service reports on. When set, the browser and API cannot change it, which keeps an open service delivering notifications for your leagues only. SQLite state defaults to `data/awaker.sqlite`, overridable with `AWAKER_DB`. The service uses Node's built-in SQLite support, which Node 22 labels experimental.
