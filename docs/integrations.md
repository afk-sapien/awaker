...
5. To receive pushes, fill in **Your phone** on the same page. No ntfy account is needed: keep the default `https://ntfy.sh` server, press **Random** for a topic, subscribe to the shown address in the ntfy app, and use **Send test**, which reports whether ntfy accepted it and why not. On a public server the topic name is the only secret, so keep it unguessable. A self-hosted server and an access token for a protected topic are optional. Changes apply immediately, without a restart. Without ntfy, scheduled reports are archived in the app.
