export const defaults={username:'',disabled:[],preferences:{},timezone:'America/Los_Angeles',daily:{enabled:false,time:'08:00'},weekly:{enabled:false,time:'08:00',day:2},alerts:{trades:false,minGain:3,improvement:2,cooldownHours:6,dailyCap:3,quietStart:'22:00',quietEnd:'08:00'}};
...
export function preferences(input){
 ...
 for(const [k,min,max]of [['tradeMinGain',1,10],['tradeMaxGap',0,1],['tradePenalty',0,1],['tradeEndWeek',17,18]])if(k in input){if(!Number.isFinite(input[k])||input[k]<min||input[k]>max||(k==='tradeEndWeek'&&!Number.isInteger(input[k])))throw bad(`Invalid ${k}`);out[k]=input[k]}
 ...
 if(out.tradeMinGain!==undefined&&![1,2,3,5,10].includes(out.tradeMinGain))throw bad('Unsupported minimum gain.');
 if(out.tradeMaxGap!==undefined&&![.15,.25,.4].includes(out.tradeMaxGap))throw bad('Unsupported value gap.');
