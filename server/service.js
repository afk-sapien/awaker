...
export function createService({store,provider,now=Date.now}){
 ...
 function envelope(data,result,warnings=[]){
  const sources=(data.sources||[]).map(s=>({...s,stale:!s.fetchedAt||now()-s.fetchedAt>s.maxAgeMs}));
  const issues=[...data.errors,...sources.filter(s=>s.stale).map(s=>`${s.name} is stale`),...warnings];
  return {schemaVersion:1,generatedAt:now(),season:data.nfl.season,week:data.week,demo:data.demo,sources,complete:issues.length===0,health:issues.length?'degraded':'healthy',warnings:[...new Set(issues)],...result};
 }
