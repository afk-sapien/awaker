...
import {activeSlots,optimize,playableIds,ruledOut} from '../../engine.js';
...
export function lineup(){
 ...
 <div class="panel table-wrap"><table class="lineup-table"><thead><tr><th scope="col">SLOT</th><th scope="col">CURRENT STARTER</th><th scope="col">START INSTEAD</th><th scope="col">GAIN</th><th scope="col">DECISION</th></tr></thead><tbody>${starters.map(({slot,index:i,projection,locked:lock})=>{
  ...
  const swap=!same&&!lock&&id,out=!lock&&projection===0&&ruledOut(S.data.players[ids[i]]);
  return `<tr class="${swap?'lineup-swap':lock?'lineup-locked':''}"><td><strong>${esc(slot)}</strong></td><td>${playerChip(ids[i],`${playerInfo(ids[i])}<div class="num small">${fmt(projection)} projected pts${out?' · ruled out':''}</div>`,'span')}</td><td>${swap?playerChip(id,`${playerInfo(id)}<div class="num small">${fmt(points)} projected pts</div>`):`<span class="muted small">${lock?'Game started':id?'No better option':out?'Nobody on your bench can play here':'No projection'}</span>`}</td><td class="num ${swap?deltaClass(delta):'muted'}">${swap?deltaLabel(delta):'—'}</td><td><span class="pill ${swap?'live':lock?'locked':''}">${lock?'LOCKED':same?'KEEP':id?'SWAP IN':out?'OUT':'NO PROJECTION'}</span></td></tr>`;
